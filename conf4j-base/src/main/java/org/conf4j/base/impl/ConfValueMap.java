...
public final class ConfValueMap extends
		ConcurrentHashMap<String, ConfValue> {
	...
	static final List<EConfigUsage> UNDEFINED_USAGE = Arrays
			.asList(new EConfigUsage[] { EConfigUsage.undefined });

	public ConfValue put(String name, String value, ESource source) {
		final ConfValue configValue = get(name);
		final String description = configValue == null ? "" : configValue
				.getDescription();
		final List<EConfigUsage> usages = configValue == null ? UNDEFINED_USAGE
				: configValue.getUsages();
		final EEnvironment env = configValue == null ? EEnvironment.prod
				: configValue.getDefaultEnv();
		return put(name, new ConfValue(value, source, description, usages,
				env));
	}
	public final void checkUsage(ConfValue value, String name, PrintStream os)
	
			throws IOException {
		final ConfValue appname = super.get(ConfElements.appname);
		final EConfigUsage usage = appname == null ? undefined : EConfigUsage
				.valueOf(appname.getValue());
		if (value == null)
			return;
		if (usage == null) {
			os.println(USAGE_0_INVALID_FOR_ELT_1.format(new Object[] { usage,
					name }));
			return;
		}
		...
		if (value.getUsages().size() == 1
				&& value.getUsages().contains(undefined))
			return;
		if (!value.getUsages().contains(usage)) {
			os.println(USAGE_0_NOT_FOUND_FOR_ELT_1_SOURCE_2
					.format(new Object[] { usage, name, value.getSource() }));
			return;
		}
	}
	...
	public final void checkUnused(PrintStream os) throws IOException {
		...
		for (Map.Entry<String, ConfValue> entry : entrySet()) {
			...
			if (entry.getValue().getAccessCount() == 0)
				os.println(USAGE_0_SEEMS_TO_BE_UNUSED_FOR_USAGE_1
						.format(new Object[] { entry.getKey(), usage }));
		}
	}
}
