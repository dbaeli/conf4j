...
public final class ConfEvaluator implements Evaluator {
    ...
    @Override
    public String eval(Map<String, ?> conf, String key, String defaultReplacement, boolean encodeHTML) {
        final ConfValue configValue = ((ConfValueMap) conf).get(key);
        if (configValue == null)
            return "${" + key + "}";
        else
            return configValue.getValue(countAccess);
    }
